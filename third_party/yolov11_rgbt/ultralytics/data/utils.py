...
def img2label_paths(img_paths, use_simotm=None, kind_label=None):
    """Define label paths as a function of image paths."""
    sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}labels{os.sep}"  # /images/, /labels/ substrings

    # RGBT模式下，yaml文件指向的是visible，但是我们需要用的标签是是infrared，所以这里替换一下
    # Grey模式下，yaml文件指向的是infrared，然后保留原来的路径然后替换即可
    # RGB模式下，yaml文件指向的是visible，然后保留原来的路径然后替换即可
    
    # 默认的标准路径转换逻辑
    label_paths = [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt" for x in img_paths]
    
    # 用户显式指定了需要的标签类型
    if kind_label in ["infrared", "visible"]:
        return [p.replace('visible', kind_label).replace('infrared', kind_label) for p in label_paths]
        
    # 根据 use_simotm 模式动态调整标签路径，或者如果路径包含 visible 但需要强制替换时
    # 注意：如果 use_simotm 未传递过来，但在路径中检测到需要处理的双模态结构，我们也可以在这里进行备用处理
    if use_simotm == "RGBT" or (use_simotm is None and any("visible" in p for p in label_paths)):
        return [p.replace('visible', 'infrared') for p in label_paths]
    
    # 对于 Gray, RGB 等其他模式，直接返回默认路径
    return label_paths
