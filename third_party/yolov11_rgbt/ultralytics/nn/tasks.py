...
from ultralytics.nn.cqs import (
CrossModalAttention,
DualModalInteractionBlock,
GetIndex,
SPPCSPC,
C2f_TextureEnhance,
EdgeGate,
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in frozenset({CrossModalAttention,DualModalInteractionBlock}) :
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c2, *args[1:]]
        elif m in frozenset({EdgeGate,FreqEnhance}):
            c1, c2 = ch[f[1]], args[0]
            ...
            args = [args[0] if len(args) > 0 else 1, [ch[x] for x in f]]
        elif m is ADD:
            c2 = max(ch[x] for x in f)
