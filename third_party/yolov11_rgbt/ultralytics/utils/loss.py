...
class EqualizedFocalLoss(nn.Module):
    """
    适配 YOLO 的 Equalized Focal Loss (EFL)
    输入格式：(pred_scores, target_scores)，均为 [N, num_classes]
    """
    ...
    def forward(self, pred_scores, target_scores):
        """
        pred_scores: [N, num_classes] 原始 logits
        target_scores: [N, num_classes] 0/1 one-hot
        """
        # 确保 target_scores 是 float 类型
        target_scores = target_scores.float()
        
        # 计算 pt = sigmoid(pred_scores) 对于正样本，对于负样本则为 1-sigmoid
        prob = torch.sigmoid(pred_scores)
        # 正样本的 pt
        pt_pos = prob * target_scores
        # 负样本的 pt (负样本标签为0，所以 pt_neg = 1-prob)
        pt_neg = (1 - prob) * (1 - target_scores)
        # 合并所有位置的 pt (属于正样本或负样本)
        pt = pt_pos + pt_neg
        
        # 计算基础 BCE loss (不带 reduction)
        bce_loss = F.binary_cross_entropy_with_logits(pred_scores, target_scores, reduction='none')
        
        # 动态 gamma：需要根据每个类别的梯度比计算
        # 这里简化处理：使用固定的 gamma，因为动态梯度计算在 YOLO 大规模 anchor 上开销很大
        # 若需要严格 EFL，可开启下面的动态部分（较慢）
        if hasattr(self, 'use_dynamic_gamma') and self.use_dynamic_gamma:
            # 计算梯度 (仅用于演示，实际训练会显著变慢)
            loss_sum = bce_loss.sum()
            grad = torch.autograd.grad(loss_sum, pred_scores, create_graph=True, retain_graph=True)[0]
            grad_pos = grad * target_scores
            grad_neg = grad * (1 - target_scores)
            sum_grad_pos = grad_pos.sum(dim=1, keepdim=True)
            sum_grad_neg = grad_neg.sum(dim=1, keepdim=True) + 1e-9
            ratio = sum_grad_pos / sum_grad_neg
            ratio = torch.clamp(ratio, 0, 1)
            gamma_dynamic = self.gamma + 2.0 * (1 - ratio)
            gamma_dynamic = gamma_dynamic.detach()
            weight = gamma_dynamic / self.gamma * (1 - pt) ** gamma_dynamic
        else:
            # 标准的 Focal Loss 权重，加上类别平衡（如果有 alpha）
            weight = (1 - pt) ** self.gamma
        
        # 应用类别权重
        if self.alpha is not None:
            # alpha 扩展到每个样本的每个类别
            alpha_weights = self.alpha.to(pred_scores.device).view(1, -1)
            # 对于负样本，权重为 1；对于正样本，乘以 alpha 中对应类别的值
            alpha_factor = target_scores * alpha_weights + (1 - target_scores) * 1.0
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        # counts = [226973, 9986, 8059, 13057, 7208]  # car, bus, freight_car, truck, van
        # total = sum(counts)
        # num_classes = len(counts)
        
        # # 计算逆平方根权重
        # alpha = np.sqrt(total / (num_classes * np.array(counts)))
        # # 归一化（可选，使平均权重为1）
        # alpha = alpha / np.mean(alpha)
        # self.bce = EqualizedFocalLoss(gamma=2.0, alpha=alpha, reduction='mean')
        self.hyp = h
    ...
    def compute_loss(self, preds, batch):
        ...
        # cls loss
        if isinstance(self.bce, (nn.BCEWithLogitsLoss, FocalLoss_YOLO,EqualizedFocalLoss)):
